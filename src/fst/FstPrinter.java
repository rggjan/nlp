...
public class FstPrinter<T extends IResultCollector> {
	private final Set<State<T>> handledStates = new HashSet<State<T>>();

	private final HashMap<State<T>, Integer> ids = new HashMap<State<T>, Integer>();
	private int nextId;
	...
	public static <T extends IResultCollector> void print(State<T> startState,
			PrintStream out) {
	...
	private void printRec(State<T> state) {
		...
		out.printf("%d [label=\"%s\"%s];\n", getId(state), Integer
				.toString(getId(state)), state.isAccepting() ? " peripheries=2"
				: "");
