...
import static fst.Utils.areEqual;
...
public class Configuration<TCollector extends IResultCollector> {
	private final Tape inputTape;
	private final Tape outputTape;
	...
	public Configuration(State<TCollector> startState, Tape lowerTape,
			Tape upperTape, TCollector collector) {
	...
	public void run() {
		// accept state before checking for recursion
		if (currentState.isAccepting()) {
			collector.success(this);
		}

		// check if the state has been seen already
		if (activeConfigurations.contains(this))
			return;
		...
		for (Link<TCollector> link : currentState.getLinks()) {
			...
			Configuration<TCollector> configuration = new Configuration<TCollector>(
					this);

			// leave the current state, cross the link and enter the target
			// state
	...
	public boolean equals(Object obj) {
		...
		if (collector != other.collector)
			return false;
		if (!areEqual(inputTape, other.inputTape))
			return false;
	...
	public int hashCode() {
		...
		if (inputTape != null)
			hash = hash * 31 + inputTape.hashCode();
